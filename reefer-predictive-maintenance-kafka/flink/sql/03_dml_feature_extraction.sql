-- Tumbling window feature extraction grouped by device_id.
-- Window size must match Python POC WINDOW_SECONDS (default 10).
-- slope_return_air_per_min is an approximation of the Python polyfit slope:
--   (max - min) return air temp scaled to per-minute rate over the window.

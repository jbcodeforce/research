...
## KMA-CHAT.sql → kma_chat (GROUP BY)

KSQL: `CREATE TABLE KMA_CHAT ... AS SELECT` with mixed bare columns and `LATEST_BY_OFFSET(...)` aggregates, grouped by `msg_from_id`, `msg_to_id`, `msg_epoch`, `msg_incoming`.
